...
const defaults = {
  ...
  complexion: "Fair", top: "T-Shirt", bottoms: "Shorts", sound: true, dialogueStep: 0, firstChoice: "",
};
...
function render() {
  switch (state.scene) {
    ...
    case "brian": renderStoryDialogue("brian-street.png", "BRIAN", [["PLAYER", `“Hey Brian, I’m gonna be a BASE jumper.”`], ["BRIAN", `“Hey, ${escapeHtml(state.name || "mate")}. Nobody cares.”`]], "dave"); break;
    case "dave": renderStoryDialogue("dave-office.png", "DAVE", [["PLAYER", `“Hey Dave, I got that report finished by one. Guess what? I’m going to be a BASE jumper.”`], ["DAVE", `“Hey, ${escapeHtml(state.name || "mate")}. Couldn’t give a shit. Get back to work.”`]], "parents"); break;
    case "parents": renderStoryDialogue("parents-car.png", "MOM & DAD", [["PLAYER", `“Hey, Mom and Dad, guess what? I’m going to train as a BASE jumper.”`], ["PARENTS", `“But, ${escapeHtml(state.name || "love")}, what about the law degree we paid for?”`], ["PLAYER", `“Stop trying to control me. I’m a BASE jumper now.”`]], "wife-promise"); break;
    case "wife-promise": renderStoryDialogue("kitchen.webp", "THE PROMISE", [["PLAYER", `“Hey everyone, guess what? I’m going to be a BASE jumper.”`], ["WIFE", `“What do you mean? That sounds incredibly dangerous. What if you get killed? You’ve got a wife and children to support.”`], ["PLAYER", `“I won’t get hurt.”`], ["PLAYER", `“I’ll be really careful.”`], ["PLAYER", `“I won’t make the same mistakes as everyone else who has died.”`], ["PLAYER", `“I won’t rush my progression.”`], ["PLAYER", `“I’ll do lots of skydiving to build up my experience level.”`], ["PLAYER", `“I’ll only jump with good people.”`], ["PLAYER", `“I’ll avoid drinking and partying and other temptations like girls and drugs.”`], ["PLAYER", `“I’ll just do it for a little while, and then I’ll stop.”`]], "first-choice"); break;
    case "first-choice": renderFirstChoice(); break; case "chapter-preview": renderChapterPreview(); break;
...
function renderStoryDialogue(asset, title, lines, nextScene) { const i = Math.min(state.dialogueStep || 0, lines.length - 1), [speaker, line] = lines[i], final = i === lines.length - 1; app.innerHTML = shell(`<article class="scene story-dialogue is-active"><img class="scene-bg" src="assets/${asset}" alt="" draggable="false"><div class="vignette"></div><div class="comic-tag">${title}</div><div class="caption dialogue chapter-dialogue"><span class="speaker-chip ${speaker === "PLAYER" ? "dad" : "mum"}">${speaker}</span>${line}</div><div class="story-control"><button class="story-button" data-dialogue-next="${nextScene}" data-dialogue-final="${final}">${final ? "CONTINUE" : "NEXT"}</button></div></article>`); }
...
function bindActions() {
  ...
  document.querySelectorAll("[data-dialogue-next]").forEach(btn => btn.addEventListener("click", () => { if (btn.dataset.dialogueFinal === "true") { state.dialogueStep = 0; go(btn.dataset.dialogueNext); } else { state.dialogueStep = (state.dialogueStep || 0) + 1; save(); sfx(); render(); } }));
  document.querySelectorAll("[data-first-choice]").forEach(btn => btn.addEventListener("click", () => { state.firstChoice = btn.dataset.firstChoice; save(); sfx("confirm"); go("chapter-preview"); }));
  ...
  document.querySelectorAll("[data-action]").forEach(btn => btn.addEventListener("click", () => {
    ...
    if (action === "start-career") { state.dialogueStep = 0; go("brian"); }
    if (action === "replay-choice") go("first-choice");
  }));
}
